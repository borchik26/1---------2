...
class _RecipesListScreenState extends State<RecipesListScreen> {
  ...
  List<String> _allFetchedRecipes = []; // Отслеживание всех загруженных рецептов
  String? _result; // Хранит результат от AI
  String? _errorMessage; // Сообщение об ошибке
  final ScrollController _scrollController = ScrollController(); // Контроллер прокрутки списка
                    ...
                    if (index < _recipes.length) {
                      return ListTile(
                        ...
                        trailing: IconButton(
                          ...
                          onPressed: () {
                            try {
                              launchYandexImages(context, _recipes[index]); // Поиск изображения рецепта в Яндекс
                            } catch (e) {
                              // Отправка ошибки в Telegram
                              TelegramHelper.sendTelegramError(
